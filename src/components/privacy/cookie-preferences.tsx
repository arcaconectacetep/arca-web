...
export function CookiePreferences() {
  ...
  return (
    <aside className="fixed inset-x-3 bottom-3 z-[80] mx-auto max-w-2xl rounded-2xl border border-line bg-paper p-5 shadow-lift" aria-labelledby="cookie-title">
      ...
      {details && <div className="mt-4 space-y-3 rounded-xl bg-canvas p-4 text-sm"><label className="flex items-center justify-between gap-4"><span><b className="block">Essenciais</b><small className="text-muted">Login, segurança e preferências de consentimento.</small></span><input type="checkbox" checked disabled /></label><label className="flex cursor-pointer items-center justify-between gap-4"><span><b className="block">Personalização</b><small className="text-muted">Tema e experiência visual neste navegador.</small></span><input type="checkbox" checked={personalization} onChange={(event) => setPersonalization(event.target.checked)} /></label><p className="text-xs text-muted">Não utilizamos cookies de publicidade ou análise. <Link className="font-bold text-brand" href="/privacidade">Ler Política de Privacidade</Link></p></div>}
      <div className="mt-4 flex flex-col gap-2 sm:flex-row sm:justify-end"><button className="btn-ghost" onClick={() => setDetails((value) => !value)}>{details ? "Ocultar opções" : "Configurar"}</button><button className="btn-secondary" onClick={() => accept("essential")}>Somente essenciais</button><button className="btn-primary" onClick={() => accept(personalization ? "personalization" : "essential")}>Salvar preferências</button></div>
    </aside>
  );
}
