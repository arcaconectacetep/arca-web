...
export default async function Page() {
  ...
  return <>
    ...
    <main id="conteudo">
      ...
      <figure className="relative mx-auto max-w-5xl px-5 pt-12">
        <div className="relative aspect-[16/9] overflow-hidden rounded-2xl bg-brand-soft sm:aspect-[16/7]">
          <Image src="/images/institutional/arca-processo.webp" fill priority sizes="(max-width: 1024px) 100vw, 984px" alt="Mesa de trabalho com computador, equipamento de rede, cabos e planejamento de uma solução técnica" className="object-cover" />
        </div>
        <IllustrativeCaption description="Cena representativa do processo de arquitetura, redes e prototipagem." />
      </figure>
      <section className="mx-auto grid max-w-5xl gap-12 px-5 py-16 lg:grid-cols-2"><div><p className="eyebrow">Objetivo geral</p><h2 className="section-title mt-3">Consolidar uma rede social acadêmica para integração e cuidado.</h2><p className="mt-4 leading-7 text-muted">A plataforma reúne disseminação cultural, comunicação pedagógica, participação estudantil, proteção socioemocional e combate sistemático ao bullying e aos preconceitos.</p></div><ul className="space-y-3">{objectives.map((item) => <li className="flex gap-3 rounded-xl bg-paper p-4 shadow-quiet" key={item}><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-success" /><span className="text-sm leading-6">{item}</span></li>)}</ul></section>
      ...
      <section className="mx-auto max-w-5xl px-5 py-16"><div className="card flex flex-col gap-6 p-6 sm:flex-row sm:items-center sm:justify-between sm:p-8"><div><p className="eyebrow">ConectaARCA em funcionamento</p><h2 className="section-title mt-2">Conheça a plataforma.</h2><p className="mt-2 max-w-xl text-sm leading-6 text-muted">A aplicação preserva o propósito do ARCA com autenticação, perfis, publicações, acessibilidade, moderação e suporte privado por protocolo.</p></div><Link className="btn-primary shrink-0" href={user ? "/inicio" : "/cadastro"}>{user ? "Acessar plataforma" : "Criar uma conta"}<ArrowRight className="size-4" /></Link></div></section>
    </main>
